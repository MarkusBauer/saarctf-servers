...
def service_stat(service):
    ...
    for line in output.splitlines():
        ...
        status_search_d = re.search(status_regx_dead, line)
        
        if status_search or status_search_d:
            status = status_search.group(1).strip() if status_search else status_search_d.group(1).strip()
            status_fail = status_search_f.group(1).strip() if status_search_f else ''
